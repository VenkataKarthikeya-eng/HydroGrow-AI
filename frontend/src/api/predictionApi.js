...
export const predictGrowth = async (params) => {
  try {
    const response = await client.post('/api/predict', params, { timeout: 8000 });
    ...
    const tempImpact = Math.max(0, (25 - Math.abs((params.air_temperature || 22.0) - 22.0) * 3));
    const ecImpact = Math.max(0, (30 - Math.abs((params.water_ec || 2.0) - 2.2) * 10));
    const phImpact = Math.max(0, (20 - Math.abs((params.water_ph || 6.2) - 6.2) * 8));
    ...
    return {
      ...
      prediction: {
        predicted_weight: calculatedYield,
        growth_category: calculatedYield > 360 ? 'Optimal Growth' : 'Moderate Growth'
      },
      factors: {
